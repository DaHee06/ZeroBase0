...
class LinkedList2 extends LinkedList {
    ...
    public void addData(int data, Integer beforeData){
        ...
        }else{  //before_data에 값이 있는 경우
            Node cur = this.head;
            Node pre = cur;
            while(cur != null){
                if(cur.data == beforeData) {
                    if(cur == this.head){  //cur이 head라면
                        this.head = new Node(data, this.head);  //새로운 노드의 다음 노드는 head
                    }else{
                        pre.next = new Node(data, cur);   //pre - data - cur 순서대로
                    }
                    break;
                }
                pre = cur;
       ...
       while (cur != null){
           if(cur.data == data){
               if(cur == this.head){
                   ...
                   pre.next = cur.next;  //pre - cur - cur.next  , pev 다음을 cur.next 값이 되게 진행
               }
               break;
           }
